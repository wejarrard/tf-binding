...
class HyperParams:
    ...
    num_epochs: int = 50
    batch_size: int = 8 if torch.cuda.is_available() else 2
...
def main(hyperparams: HyperParams) -> None:
    ...
    model = DeepSeq(config, train_loader_length=len(train_loader), state_dict=f"{data_dir}/pretrained_weights.pth")
    ...
    progress_bar_callback = TQDMProgressBar(refresh_rate=500)
